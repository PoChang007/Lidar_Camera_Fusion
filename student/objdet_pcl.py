...
def key_callback(vis):
    global call_back_func_used
    call_back_func_used = True
...
def show_pcl(pcl):
    ...
    vis.create_window("Visualize lidar point-cloud. Press right arrow or space button to exit")
    ...
    global call_back_func_used
    call_back_func_used = False

    while True:
        vis.update_geometry(pcd)
        vis.poll_events()
        vis.update_renderer()
        if call_back_func_used is not False:
            break
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    ## step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur by centering the forward-facing x-axis 
    ...
    intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 3] / float(np.percentile(lidar_pcl_top[:, 3], 99) - np.percentile(lidar_pcl_top[:, 3], 1))
    
    ## step 5 : temporarily visualize the intensity map using OpenCV to make sure that vehicles separate well from the background
    img_intensity = intensity_map * 256
    img_intensity = img_intensity.astype(np.uint8)
    cv2.imshow('Intensity Map. Press ESC or Space button to exit', img_intensity)
    cv2.waitKey(0)
    ...
    cv2.imshow('Height Map. Press ESC or Space button to exit', img_height)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
